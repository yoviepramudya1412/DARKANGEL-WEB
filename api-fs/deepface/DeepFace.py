...
def verify(img1_path, img2_path = '', model_name = ['VGG-Face', 'Facenet'], distance_metric = 'cosine', model = None, enforce_detection = True, detector_backend = 'opencv', align = True, prog_bar = True, normalization = 'base', voting_method = "based on threshold"):
	...
	disable_option = (False if len(img_list) > 0 else True) or not prog_bar # > 1
...
# 			idx_number = instance.index([instance[0], instance[1]])
			...
			for i in  model_names:
				...
				img1_representation = represent(img_path = img1_path
						, model_name = i, model = custom_model
				...
				for j in metrics:
					...
					if i != 'Ensemble':
						...
						resp_obj = {
							...
							, "duration": duration  # Test timer
						}

						if bulkProcess == True:
							resp_objects.append(resp_obj)
						else:
							if type(model_name) == list: #test juga
								if voting_method != None:
									resp_objects.append(resp_obj)
								else:
									raise ValueError("Invalid voting method passed to verify function: ", voting_method)
							else:
								return resp_obj
			...
			if type(model_name) == list:   
				if voting_method != None:
					vote_result = Voting.vote(img1_path, img2_path, resp_objects, distance_metric, voting_method)
					vote_toc = time.time() # Test timer
					vote_result['duration'] = vote_toc - tic
                    
					if bulkProcess == True:
						voting_result["pair_%d" % (index + 1)] = vote_result
					else:
						return vote_result
				else:
					raise ValueError("Invalid voting method passed to verify function: ", voting_method)
